...
%MULTIPLEOBJECTTRACKINGTOOL User interface for tracking multiple objects in
%image sequences.
...
% Copyright 2016-2019 Matthew R. Eicholtz
    ...
    properties (Access=private,Constant) %default parameter values
        % Home\Options
    ...
    methods
        function this = MultipleObjectTrackingTool(varargin)
            ...
            this.GroupName = regexprep(tempname,'.*\',''); %random unique name for each instance of the app
            ...
            this.HomeTab        = this.ToolGroup.addTab('Home','Home');
            ...
            this.VideoControlsSection   = gui.VideoManager(this.HomeTab);
            this.ZoomPanSection         = gui.ZoomPanManager(this,'Home','ImageHandle');
            ...
            md.setGroupLocation(this.GroupName,loc);
...
%                     this.RoiTab.reset();
                ...
                case 'start tracking'
                    disable(this.AllHandles);
                    disable(this.VideoControlsSection);
%                     disable(this.ZoomPanSection);
    ...
    methods (Access=private)
        ...
        function updatehandles(this)
            ...
            this.TabHandles.Home = [ ...
                this.SectionHandles.File, ...                this.VideoControlsSection.Handles, ...                this.ZoomPanSection.Handles, ...
    ...
    methods (Access=private) %layout sections
        %******************************************************************
        ...
        function layoutOptionsSection(this)
            ...
            % Setup toolstrip panel
            panel = toolpack.component.TSPanel('f:p,3dlu','f:p:g,f:p:g,f:p:g'); %(columns,rows)
            panel.add(this.RoiCheckBox,         'xy(1,1)');
            panel.add(this.DetectionCheckBox,   'xy(1,2)');
            panel.add(this.TrackingCheckBox,    'xy(1,3)');
...
%             fprintf('%d\n',t1);
        ...
        function listenerTrackingFlag(this,~,~)
            ...
            if ~isempty(X)
                ...
                btui.utils.track(this,I,centroids,bboxes);
                
            else
                this.status('No detections loaded. Click "Load Detections" before starting tracking.');
                pause(2);
            end
    ...
    methods (Access=private) %other processing methods
        %******************************************************************
