function tracks = multipleObjectTracking()
...
% file = fullfile('resources','demos','motion03.tif');
% file = fullfile('resources','private','1927 hemi-3 Substack(250-1000).tif');
% file = fullfile('resources','private','1929 cont-1Substack (1-100).tif');
file = fullfile('resources','private','Df1 3171 cont left-2.tif');
% file = fullfile('resources','private','Df1 3197 hemi left-1.tif');
% file = fullfile('resources','private','Df1 3197 hemi left-1_2.tif');
% file = fullfile('resources','private','Df1 3403 cont left-2.tif');

file = 'BeadTracker\resources\demos\motion03.tif';

obj = setupSystemObjects(file);
...
for ii=1:size(obj.imageSequence,4)
    frame = readFrame();
    ...
    fprintf('frame %d\t #tracks %d\n',ii,length(tracks));
    
    pause(0.01)
end

    function obj = setupSystemObjects(file)
    %% Create System Objects
        ...
        obj.imageSequence = imframe(file,'all','class','uint8');
        obj.imageSequence(:,:,2,:) = obj.imageSequence(:,:,1,:);
        obj.imageSequence(:,:,3,:) = obj.imageSequence(:,:,1,:);
        obj.currentFrame = 0;
        
        % Create object for optical flow
        obj.opticalFlow = opticalFlowHS();
    ...
    % Read the next video frame from the video file.
        ...
        frame = obj.imageSequence(:,:,:,obj.currentFrame);
    end
...
%         mask = obj.detector.step(frame);
        h = fspecial('log',[7 7],2);
        I = imfilter(im2double(frame(:,:,1)),h,'replicate','same');
        mask = I<-0.01;
%         if obj.currentFrame==1
%             mask = false(size(frame(:,:,1)));
%         else
%             f = obj.opticalFlow.estimateFlow(frame(:,:,1));
%             mask = f.Magnitude>=0.05*max(f.Magnitude(:));
%         end
