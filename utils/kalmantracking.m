function tracks = kalmantracking(I,centroids,bboxes,varargin)
%KALMANTRACKING Track detected objects across frames in an image sequence
...
for ii=1:size(I,4)
    ...
    % --------------------------------------------
    if ii==1
        himage = imshow(im2uint8(frame));
    else
        hfigure = gcf;
        pos = hfigure.Position;
        
        haxes = gca;
        xlim = haxes.XLim;
        ylim = haxes.YLim;
        
        himage.CData = im2uint8(frame);
        
        hfigure = gcf;
        hfigure.Position = pos;
...
%PARSEINPUTS Custom input parsing function.
    ...
    p.addParameter('maxNumPoints',default.maxNumPoints,...
        @(x) validateattributes(x,{'numeric'},{'scalar','real','>',0,'nonempty','nonsparse'}));
