function blobs = blobdetection(varargin)
%BLOBDETECTION Extract blobs in an image sequence using LoG filters.
%   BLOBDETECTION(FILENAME) reads an input image sequence and detects
%   circular blobs in each frame using a set of Laplacian of Gaussian (LoG)
%   filters.
%
%   BLOBDETECTION(...,'ParamName',ParamValue,...) uses additional parameter
...
%                       Default: 'bright'
%
%       'SaveOutput'    Logical scalar indicating whether to save the
%                       output to file or not. If true, the output filename
%                       is '<input_filename> blobs.mat'.
%
%                       Default: true
%
...
%% Parse inputs
[filename,polarity,save2file,radii,sens,verbose] = parseinputs(default,varargin{:});
if isempty(filename)
...
blobs = cell(n,1); %initialize blob cell array
for ii=1:n
    status('detect',ii);
...
if save2file
    ...
    status('save');
    save(savefile,'blobs','polarity','radii','sens');
end
...
%STATUS Print text in Command Window.
    ...
    switch message
        ...
        case 'save'
            fprintf('%s\tSaving detected blobs to file: %s\n',datestr(now),savefile);
...
function varargout = parseinputs(default,varargin)
    ...
    p.addParameter('saveoutput',default.SaveOutput,...
    ...
    [filename,polarity,save2file,radii,sens,verbose] = struct2vars(p.Results);
    ...
    varargout = {filename,polarity,save2file,radii,sens,verbose};
end
