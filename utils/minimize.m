function [X, fX, i] = minimize(X,f,L,varargin)
...
if max(size(L)) == 2, red=L(2); L=L(1); else red=1; end
if L>0, S='Linesearch'; else S='Function evaluation'; end 

i = 0;                                            % zero the run length counter
ls_failed = 0;                             % no previous line search has failed
[f0 df0] = feval(f, X, varargin{:});          % get function value and gradient
fX = f0;
i = i + (L<0);                                            % count epochs?!
s = -df0; d0 = -s'*s;           % initial search direction (steepest) and slope
x3 = red/(1-d0);                                  % initial step is red/(|s|+1)

while i < abs(L)                                      % while not finished
  i = i + (L>0);                                      % count iterations?!

  X0 = X; F0 = f0; dF0 = df0;                   % make a copy of current values
  if L>0, M = MAX; else M = min(MAX, -L-i); end
  ...
  if abs(d3) < -SIG*d0 && f3 < f0+x3*RHO*d0          % if line search succeeded
    ...
    if ls_failed || i > abs(L)         % line search failed twice in a row
